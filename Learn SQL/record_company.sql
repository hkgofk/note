...
SELECT DISTINCT B.name 'Brand Name'
FROM bands B INNER JOIN albums A ON B.id = A.band_id
-- Get all Bands that have No Albums
SELECT name 'Brand Name'
FROM bands
WHERE id NOT IN (
				SELECT DISTINCT B.id 'Brand Name'
				FROM bands B INNER JOIN albums A ON B.id = A.band_id
				)
-- Get the Longest Album
SELECT TOP (1) A.name 'Name', A.release_year 'Release Year', SUM(S.length) 'Duration'
...
INSERT INTO bands VALUES('Favorite Band')
SELECT * FROM bands
INSERT INTO albums VALUES('Favorite albums', 2022, 8)
select * from albums
-- Delete the Band and Album you added in #8
DELETE FROM albums
WHERE name = 'Favorite albums'
DELETE FROM bands
WHERE name = 'Favorite Band'
-- Get the Average Length of all Songs
SELECT AVG(length) 'Average Song Duration'
FROM songs
-- Select the longest Song off each Album
SELECT A.name 'Album', A.release_year 'Release Year', MAX(S.length) 'Duration'
FROM albums A INNER JOIN songs S on A.id = s.album_id
GROUP BY A.name, A.release_year
-- Get the number of Songs for each Band
SELECT B.name 'Band', COUNT(*) 'Number of Songs'
FROM bands B INNER JOIN albums A ON B.id = A.band_id INNER JOIN songs S ON A.id = S.album_id
GROUP BY B.name